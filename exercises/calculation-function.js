...
function calculateTotalIncome(month) {
  ...
  for (const transaction of sampleTransactions) {
    if (transaction.type === "income") {
      if (month && transaction.date.includes(month)) {
...
function calculateNetBalance(month) {
  ...
  const expense = calculateTotalExpenses(month);
...
function calculateSpendingByCategory(month) {
  ...
  for (transaction of sampleTransactions) {
    if (transaction.type === "expense" && transaction.date.includes(month)) {
...
function calculateAverageTransaction(type, month) {
  ...
  // 1. Loop through transactions
  for (const transaction of sampleTransactions) {
    if (transaction.type === type && transaction.date.includes(month)) {
      total += transaction.amount;
      count++;
    }
  }
...
function calculateSavingsRate(month) {
  ...
  return savingsRate > 0 ? savingsRate : 0; // Replace with your calculation
}
...
function getMonthSummary(month) {
  ...
  const savingsRate = parseFloat(calculateSavingsRate(month).toFixed(2));
...
function findTransactionsAboveAmount(amount, month) {
  ...
  const array = [];
  for (transaction of sampleTransactions) {
    if (transaction.date.includes(month)) {
